...
const UploadProject = () => {
  const {setUploadingState, uploadItems, setUploadItems } = useContext(AppContext);
  ...
  const uploadingOpreation = () => {
    ...
    if (projectNamePrompt) {
      ...
      const projectExistCheck = () => {
        ...
        if (projectExist) {
          function filterItems(type) {
            return projects[projectExist].filter((item) =>
              item.includes(type)
            );
          }
          return {
      ...
      console.log(projectExistCheck())


      /*
        if (project is exist) {
          => get all project files to index videos and images
        } else {
          => create new project and start index will be 1 for videos & images
        }
      */
