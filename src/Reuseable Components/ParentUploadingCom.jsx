...
const ParentUploadingCom = ({ uploadingOpreation }) => {
  const { loadingState, uploadItems, setUploadItems, setCurrUploadingIndex } =
  ...
  useEffect(() => {
    ...
    !loadingState && setCurrUploadingIndex(uploadItems.length - 1);
  }, [uploadItems.length]);
        ...
        >
          {pathName.includes("uploadToProject") && (
            <h1 className="dark:text-white">
              upload to project{" "}
              <span className="!select-all">{`${folderName}`}</span>
            </h1>
          )}
          <FontAwesomeIcon
